...
try:
    ...
    PASSWORD = config.get("dashboard_password", "admin")
except Exception:
    TOKEN = ""
    ALLOWED_CHAT_ID = ""
    USERNAME = ""
...
def handle_addkeyword(message):
    ...
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3:
        bot.reply_to(message, "Usage: /addkeyword <category> <word>\nExample: /addkeyword food dominos")
        return
        
    category = parts[1].lower()
    new_keyword = parts[2].lower()
    ...
    if new_keyword not in cat_keywords[category]:
        ...
        bot.reply_to(message, f"🧠 Got it! I will now recognize '{new_keyword}' as a '{category}' expense.")
    else:
        bot.reply_to(message, f"I already know '{new_keyword}' is for {category}!")
